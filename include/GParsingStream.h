// Основной механизм взят с библиотеки GParsingStream от AlexGyver. Я лишь обернул его в класс для удобства
class PStream{
private:
...
char _startSMBL;
public:
PStream(char startSMBL){
  _startSMBL = startSMBL;
}
void parsingStream(int *incData) {
  if (Serial.available() > 0) {
    ...
    if (incomingByte == _startSMBL) {                // если это $
